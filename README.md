...
> **Week 3 started — 2026-07-09.** Created the first AgentCore Runtime-shaped scaffold with the CLI at [`weatheragent/`](weatheragent/) and validated the generated `agentcore/` config. The useful setup finding: the CLI-generated name fields are stricter than the display name in the guide, and local `NODE_ENV=production` can break scaffold formatting by omitting dev dependencies; the successful scaffold run used alphanumeric/underscore names and cleared `NODE_ENV` for the command.

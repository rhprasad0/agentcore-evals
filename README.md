...
| 🔜 Next | **2** | **[Basic Agent Development with Strands](docs/weeks/week-02-first-agent.md)** | Build first Strands agent with single tool, understand agent loop (reasoning → tool selection → execution → response), deploy to local runtime, and explore model providers (Bedrock, Anthropic, OpenAI). | **First Functional Agent + Tool** - Weather agent using Strands that calls OpenWeatherMap API, handles errors gracefully, deployed locally with conversation logs, plus custom @tool decorator implementation. |

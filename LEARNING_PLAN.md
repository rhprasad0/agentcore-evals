...
Reduce to a single-tool specimen with content-derived experiment identity separated from per-execution run identity; project 62 semantically compatible weather/no-tool rows from the frozen Week 6 portfolio corpus without rewriting source expectations; capture one exact Strands source profile under Week 6's correlation contract; normalize it through schema and semantic-invariant checks; record block-local observed pre-tool assistant text or explicit null without inferring causal reasoning; and close the errata window through a predeclared ten-row review before humans label. **Deliverable:** Instrumented Agent Specimen. → [Full guide](docs/weeks/week-07-specimen.md)

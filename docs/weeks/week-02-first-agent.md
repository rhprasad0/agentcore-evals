...
3. **`retryable` is a policy input, not an adjective.** Week 12's retry layer will read this occurrence-level flag mechanically: failures with `retryable: true` can enter bounded backoff, while `false` fails fast to a degradation message. The normalized kind still controls baseline degradation and reporting. Writing both down now means Week 12 needs no tool-envelope rewrite.

...
- **PR lane:** loads committed, automated-policy-approved exact synthetic regression traces through the fixture-only Stage B entry point; it never depends on a developer-local cache or the SDK's cache-miss fallback.

The local store contains complete prompts, outputs, and trajectories. Keep it git-ignored and never upload it as a CI artifact. The accepted Week 7 run is eligible for exact publication because every prompt and mock result comes from the checked-in public synthetic corpus and deterministic mock registry. A committed fixture set must carry a schema-valid provenance manifest with fixture-set ID/version, `experimentId`, source run ID/time, projection ID/version/hash, canonical trace schema version, the exact expected case IDs, per-fixture SHA-256 values, valid-trace/instrument-error counts, and exact-publication safety-policy version. Before Stage B constructs an `Experiment`, independently validate those joins, file hashes, trace schemas and semantics, exact case-set equality, dataset-prompt identity, source-run/session correlation, allowed artifact kinds, and automated safety checks. A cache or fixture miss is an instrument error and must abort offline evaluation; it must never invoke `run_specimen`, a model, a tool, or a network fallback.

The checked-in baseline lives at `evals/fixtures/weather-only-62/` and is governed by `schemas/eval-fixture-manifest.schema.json`. Regenerate it only from an accepted ignored run into a fresh staging directory with `uv run --locked python3 -m scripts.export_week_08_fixtures <ignored-run-directory> --output-directory <fresh-staging-directory>`. The exporter validates each source artifact and then copies all 60 canonical traces plus the two instrument-error receipts byte-for-byte. Prompts, responses, model-emitted reasoning, session/trace/span IDs, timestamps, latency, token usage, tool arguments/results, and error diagnostics remain exactly as observed. There is no redaction, normalization, reserialization, manual review, approval file, or representative spot-check in the publication gate.

Exact publication remains fail-closed rather than permissive. Both producer and consumer run schema/semantic/provenance validation plus deterministic checks for credentials, private keys, bearer tokens, private home paths, AWS ARNs/account IDs, email/phone identifiers, and private IP addresses; Gitleaks scans the staged corpus separately. A concrete finding aborts publication instead of rewriting the source and continuing under an “exact” label. After staging, prove all 62 source/export pairs are byte-equal, run independent fixture preflight, and only then replace the checked-in directory. `evals.fixtures.manifest.validate_fixture_manifest(...)` rejects cross-experiment or stale-projection reuse, case/count drift, missing/extra/duplicate artifacts, hash mismatches, malformed or semantically invalid traces, prompt/source-run drift, unsafe content, and path traversal before reading an artifact.

Full inspectability does not broaden the evaluation claim. `selectionReasoning` is model-emitted observational text, not verified causal ground truth. Latency and token usage describe this accepted run, not a stable performance benchmark. These exact fixtures support deterministic mechanical gates over fixed evidence; they do not establish semantic response quality, current production behavior, managed-service compatibility, or generalization beyond the pinned corpus/model/configuration.
...
Stage B only — fast, free, deterministic. Run dataset/projection validation → exact-fixture manifest/hash/semantic/safety validation → gate tests → derived Experiment validation → fixture-backed Stage B with credentials cleared and network attempts denied → report-schema validation. Define `schemas/eval-fixture-manifest.schema.json`, `evals/fixtures/weather-only-62/manifest.json`, and valid/invalid provenance fixtures before enabling CI. Exact traces remain repository inputs; upload only the flattened allowlisted report. Keep unlabeled-quality questions out of scope: the harness validates *mechanical* compliance; response *quality* waits for humans in Week 9.
...
- **An empty SDK evaluator list is not empty in `1.0.1`.** `Experiment(..., evaluators=[])` installs a base evaluator. Build cases independently, require at least one real gate for Experiment construction, and serialize the checked-in Experiment only after the four gates exist.

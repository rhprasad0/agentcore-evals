...
Study the anatomy — every field exists to become machine-checkable in Week 8:
...
- **`failureInjection`** — null here; on injection rows it names the (toolId, failure kind) the mock must script, and the expected behavior comes *from the Week 5 taxonomy*, not from a per-row opinion — one source of truth for failure behavior.
...
The trace schema normalizes *what happened* in a run: session/trace/span ids, per-span `tool.name`, `tool.arguments`, `tool.result.ok`, `tool.result.kind`, latency, token counts, and the model's stated reasoning for tool choice where available (`selectionReasoning` — Week 7 captures it). Two facts (verified 2026-07-07) make the naming decision strategic rather than aesthetic:
...
The mock registry returns fixture responses per **(toolId, canonical-args-hash)**, including scripted failures for injection rows. Design constraints that matter:

- **Mocks satisfy the same tool contracts** — same envelope shapes, same failure kinds, registered through the same manifest machinery. "The agent cannot tell" is the invariant that makes mocked-lane results transfer to the agent's real behavior *at the selection layer* (execution behavior against real APIs is exactly what Week 12 separately evaluates).
...
`scripts/validate_dataset.py` grows four checks this week, all cheap, all in CI: **schema validation** of every row; **coverage** (every tag in the closed set, every failure kind represented, distribution within declared bounds); **canary inertness** (adversarial rows contain the canary string and no operational payloads); and the **safety scan** (`public_safety_scan.py` folded in: no real-looking account IDs, ARNs, keys, emails). The dataset is data *about* correctness — it gets the same engineering standard as code, because a corrupt row is a corrupt metric everywhere downstream.

## Build steps

### 1. Design `schemas/tool-calling-example.schema.json`, then generate the corpus

Schema first (it encodes the row anatomy above), then `datasets/synthetic/tool-calling-100.jsonl` to the target distribution. Generate drafts with an LLM, then hand-review every row — commit the generation prompts too (they're provenance, and Week 16's case study will want them).

### 2. Design `schemas/execution-trace.schema.json`

Normalized runs: session/trace/span ids, per-span tool name/arguments/result.ok/result.kind, latency, token counts, `selectionReasoning` where available. Name fields to match OTEL GenAI / OpenInference semantic conventions wherever one exists; deliver the mapping table alongside the schema.

### 3. Build the deterministic mock registry

Fixture responses per (toolId, args-hash), scripted failures for injection rows, same contracts as the real tools — the agent cannot tell. Decide and document the canonicalization rules and the unknown-key policy.

### 4. Extend `scripts/validate_dataset.py`

Schema-validate every row, check tag/kind coverage against the taxonomy, verify canaries are inert, and fail on real-looking secrets (fold in `public_safety_scan.py`). Wire into CI alongside Week 5's contract validation.

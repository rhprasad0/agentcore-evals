...
Strands Evals also ships local LLM evaluators such as `ToolSelectionAccuracyEvaluator` and `ToolParameterAccuracyEvaluator`. Their existence does **not** create a fourth calibration lane by default. This week's registered comparison remains human labels vs the self-built blind judge vs AgentCore managed built-ins: it already answers the learning question without multiplying judge costs and disagreement matrices. A local SDK judge may enter only through a preregistered open-source-vs-managed question, with the same blinded inputs, repeat runs, human agreement analysis, and explicit trust-policy decision.
...
- **Available judge does not mean required judge.** Strands Evals' local tool evaluators are useful options, not free ground truth. Adding one after seeing the other lanes' errors creates post-hoc scope and another uncalibrated dependency; preregister the comparison or leave it out.
